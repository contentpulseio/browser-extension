...
async function openAndFill(article, preferredTabId = null) {
  ...
  if (isMedium) {
    ...
    const tableImages = await tableImagesPromise;
    ...
    const mediumImageMeta = [
      ...(heroHtml ? [{ caption: heroCaption, alt: heroAlt }] : []),
      ...mediumInlineImages,
    ];
    const bodyText = stripToText(mediumBodyHtml);
    ...
    queryTargetTab((activeTab) => {
      ...
      if (activeTab && isMediumEditorUrl(activeTab.url)) {
        ...
        mediumPageFill(activeTab.id, title, mediumBodyHtml, bodyText, mediumImageMeta).then(() => mediumAfterEditorFill(activeTab.id));
        return;
      }
      ...
      chrome.tabs.create({ url: editorUrl }, (tab) => {
        ...
        const listener = (tabId, changeInfo) => {
          ...
          setTimeout(() => {
            ...
            mediumPageFill(targetTabId, title, mediumBodyHtml, bodyText, mediumImageMeta).then(() => mediumAfterEditorFill(targetTabId));
          }, 2000);
        };
  ...
  if (isSubstack) {
    ...
    const substackInlineImages = buildInlineImageMeta(bodyHtml, tableImages);
    const bodyText = stripToText(substackBodyHtml);
    ...
    const doFill = async (tabId) => {
      ...
      await substackPageFill(tabId, title, subtitle, substackBodyHtml, bodyText, substackInlineImages.length, article?.tags || []);
  ...
  const inlineImages = buildInlineImageMeta(bodyHtml, tableImages);
  ...
  const afterFill = async (tabId) => {
    ...
    if (inlineImages.length) await pageFillInlineImages(tabId, inlineImages);
  ...
  queryTargetTab((activeTab) => {
    ...
    if (activeTab && isEditorUrl(activeTab.url)) {
      ...
      pageFill(activeTab.id, title, linkedinBodyHtml, '', false, clipboardPrepared).then(() => afterFill(activeTab.id));
    ...
    getKnownPublisherUrn(publishAs).then((knownUrn) => {
      ...
      const urn = linkedinEditorAuthorUrn(configuredUrn || knownUrn || '');
      ...
      chrome.tabs.create({ url: editorUrl }, (tab) => {
        ...
        const listener = (tabId, changeInfo) => {
          ...
          pageFill(targetTabId, title, linkedinBodyHtml, '', true, clipboardPrepared).then(() => afterFill(targetTabId));
